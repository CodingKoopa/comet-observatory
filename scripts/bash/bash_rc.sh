...
function handle_errors() {
  {
    if [ $ret -ne 0 ]; then
      local i=0
      local num_times=$((1 + RANDOM % 3))
      while [ $i -lt $num_times ]; do
        mpv --really-quiet --volume=90 boom.m4a >/dev/null &
        sleep 0.2
        i=$((i + 1))
      done
    fi
  ...
  # [?2004h[koopa@comet-observatory ~]$ ls
  # [?2004l
  # [?2004h[koopa@comet-observatory ~]$ a
  # [?2004l
  # bash: a: command not found
  # [?2004h[koopa@comet-observatory ~]$

  if [ -f $cmd_file ]; then
    lc=$(wc -l <$cmd_file | cut -f1 -d" ")
    if [ "$lc" -ge 2 ]; then
      # hell
      sed -E '2d;$d;s/[[:cntrl:]]//g;s/\[\?2004[lh]//g;s/ -- //g;s/: /\n/g;s/-/dash /g' $cmd_file >$cmd_file-actual
      # tail -c +2 $cmd_file-actual-tmp >>$cmd_file-actual
      curl --silent --request POST --data-urlencode "req_text=$(cat $cmd_file-actual)" 'https://api16-normal-useast5.us.tiktokv.com/media/api/text/speech/invoke/?text_speaker=en_us_002' | jq .data.v_str | base64 -di | mpv --really-quiet -
    fi
    # cat $cmd_file >$cmd_file-actual
    # if [ "$lc" -ge 4 ]; then
    # echo
    # sed "1,+1d;$lc,\$d" $cmd_file >>$cmd_file-actual
    # echo sed "1,+1d;$(($(wc -l <cmd-output) - 1)),\$d" actual-cmd-output >cmd
    # cat cmd-output >&2
    # fi
  fi
  echo "" >$cmd_file
...
function prompt() {
  ...
  # Redirecting stdout to /dev/null is necessary for the function to not block:
  # https://unix.stackexchange.com/a/218519.
  handle_errors &
  # Prevent the job completion message from being printed: https://stackoverflow.com/a/51061046.
  disown
