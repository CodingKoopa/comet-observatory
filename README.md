...
- The entry points for everything are located in [`bin`](bin). Resultantly, all of the Bash scripts in [`bin`](bin) should be considered as libraries that export functionality that can be used by other scripts.
- The scripts are generally modular. For libraries sourced at the top of a script, they should be copied over, or their functionality reimplemented.
